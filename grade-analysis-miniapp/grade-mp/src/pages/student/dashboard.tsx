...
function StudentView() {
  ...
  useEffect(() => {
    ...
    gradeApi.getMyGrades().then((data: any[]) => setRecentGrades(data.slice(0, 8)));
  }, []);

  return (
    <View className='container'>
      <Text className='title'>你好，{user?.real_name}</Text>
      ...
      <View className='stat-row'>
        ...
        <View className='stat-card'><Text className='stat-value'>{stats?.lowest ?? '-'}</Text><Text className='stat-label'>最低分</Text></View>
        <View className='stat-card'><Text className='stat-value'>{stats?.count ?? 0}</Text><Text className='stat-label'>考试次数</Text></View>
      </View>
      <View className='card' style={{ marginTop: '24px' }}>
        <Text className='subtitle'>最近成绩</Text>
        {recentGrades.length > 0 ? recentGrades.map((g, i) => (
          <View key={i} className='list-item'>
            <View><Text style={{ fontWeight: 'bold' }}>{g.subject_name}</Text><Text className='text-secondary' style={{ marginLeft: '16px' }}>{g.exam_name}</Text></View>
            <Text className={`tag ${g.score >= 90 ? 'tag-green' : g.score >= 60 ? 'tag-blue' : 'tag-red'}`}>{g.score}</Text>
          </View>
      ...
      <View className='card'>
        ...
        <View style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
          {[{ label: '排名', url: '/pages/student/ranking' }, { label: '趋势', url: '/pages/student/trends' },
            ...
            <View key={item.label} className='btn-outline' style={{ flex: 1, minWidth: '140px' }}
    ...
    <View className='container'>
      ...
      <View className='card' style={{ marginTop: '24px' }}>
        ...
        {distribution.length > 0 ? distribution.map((d: any) => (
          <View key={d.range_label} className='list-item'><Text>{d.range_label}</Text><Text style={{ fontWeight: 'bold' }}>{d.count}人 ({d.percentage}%)</Text></View>
